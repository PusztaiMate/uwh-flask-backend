...
from project.api.models import Player, Club
...
def test_all_clubs(test_app, empty_db):
    c1, (p1, p2) = add_club_with_players("Club GETALL_1", ["Jakab Gipsz", "Janos Gipsz"])
    c2, (p3,) = add_club_with_players("Club GETALL_1", ["Julia Gipsz"])
    client = test_app.test_client()
    ...
    assert len(data[1]["players"][0]["fname"]) == p3.fname
    assert data[0]["name"] == c1.name
    assert data[1]["name"] == c2.name


def test_get_single_club(test_app, empty_db):
    add_club_with_players("Club GETALL", ["Jakab Gipsz", "Janos Gipsz"])
